...
import { AI_MODELS, parseResume } from './aiGenerate';
import { useResumeStore } from '../../store/useResumeStore';

/**
 * AI 导入简历（BYOK：Bring Your Own Key）。
 * 粘贴一份任意格式的简历纯文本，浏览器直连 Anthropic 解析成结构化简历，
 * 作为一份新的本地草稿写入（可在简历切换栏里查看、再编辑 / 导出 / 发布）。
 *
...
const AiImportPanel: React.FC<AiImportPanelProps> = ({ onClose }) => {
  ...
  const canImport = !!apiKey.trim() && !!text.trim() && !loading;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // 生成一份新的导入草稿
      const newId = `import-${Date.now().toString(36)}`;
      setDraft(newId, {
        ...result,
        id: newId,
        label: result.label || result.basics.name || '导入的简历',
      });
      setActiveId(newId);
      onClose();
    } catch (e) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden my-8">
        <div className="sticky top-0 bg-white border-b p-5 flex items-center justify-between">
          <h2 className="flex items-center gap-2 text-lg font-bold text-gray-900">
            <Icon name="file-import" className="text-emerald-500" />
            导入简历
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-start gap-3 rounded-xl bg-emerald-50 border border-emerald-100 p-4">
            ...
            <p className="text-sm text-emerald-900/80 leading-relaxed">
              把旧简历 / 任意格式的简历文本粘贴到下方，AI 会解析成可编辑的结构化简历，
              作为一份<span className="font-medium">新的本地草稿</span>
              （可在简历切换栏里查看、再编辑 / 导出 / 发布）。
            </p>
          </div>
          ...
          <label className="block">
            ...
            <textarea
              ...
              rows={9}
              placeholder={'粘贴简历内容，例如：\n张三 前端工程师\n邮箱 zhangsan@example.com\n工作经历：2021-至今 某公司 前端开发，负责…\n技能：React、TypeScript…'}
              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 resize-y"
            />
          </label>
          ...
          <div>
            ...
            <input
              ...
              placeholder="sk-ant-..."
              autoComplete="off"
              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm font-mono outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
            />
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            >
              <Icon name={loading ? 'spinner' : 'file-import'} spin={loading} />
              {loading ? '解析中…' : '解析并导入'}
            </button>
          </div>
        </div>
