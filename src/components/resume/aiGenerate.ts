...
type ResumeData = { label:string; target?:string; template?:'classic'|'sidebar'|'compact'; theme?:'blue'|'emerald'|'violet'|'rose'|'slate'|'amber'|'cyan'|'fuchsia'; settings?:{fontScale?:number;lineHeight?:number;blockGap?:number;pageMargin?:number;fontFamily?:'default'|'song'|'hei'|'kai'|'serif'}; sections?:{key:'summary'|'education'|'work'|'internship'|'projects'|'skills'|'awards'|'certificates'|'languages'|'activities'|'interests';title?:string;hidden?:boolean}[]; basics:{name:string;title?:string;email?:string;phone?:string;wechat?:string;location?:string;birth?:string;political?:string;website?:string;github?:string;summary?:string;photo?:string}; education?:{school:string;college?:string;degree?:string;major?:string;period?:string;gpa?:string;courses?:string;detail?:string}[]; work?:{company:string;position?:string;period?:string;location?:string;highlights?:string[];projects?:Project[]}[]; internship?:{company:string;position?:string;period?:string;location?:string;highlights?:string[];projects?:Project[]}[]; projects?:Project[]; skills?:string; awards?:{title:string;issuer?:string;date?:string}[]; certificates?:{name:string;issuer?:string;date?:string}[]; languages?:{name:string;level?:string}[]; activities?:{name:string;role?:string;period?:string;highlights?:string[]}[]; interests?:string[] };
规则：保留候选人真实信息，不要编造经历或数字；针对 JD 调整措辞与条目顺序，突出匹配点；要点简洁有力，可用 **粗体** 强调关键成果/数字；skills 是一段 Markdown 富文本（同 summary），按类别分组，如「**编程语言**：C++、Python」；语言与 JD 保持一致（默认中文）。若同一公司有多个项目，用 work[].projects 承载；在校/实习经历放 internship（结构同 work），正式工作放 work；不要臆造 photo（证件照由用户上传）；sections/settings 保持用户原值，除非 JD 明显需要调整模块顺序。`;
...
- 翻译所有面向人类阅读的文本为英文：label、target，basics 的 title/summary/location，education 的 school/college/degree/major/courses/detail，work 的 company/position/location/highlights 及其内嵌 projects，internship 的 company/position/location/highlights 及其内嵌 projects，projects 的 name/role/highlights，skills（一段 Markdown 富文本字符串），awards 的 title/issuer，certificates 的 name/issuer，languages 的 name/level，activities 的 name/role/highlights，interests，以及 sections 里的自定义 title；
- 保留专有名词与技术栈原名（如 React、TypeScript、Kubernetes、Vue、GitHub 等）不翻译；
- 以下字段原样保留不动：id、basics.name（人名可转为拼音或英文写法）、email、phone、wechat、birth、political、website、github、photo、avatar、所有 link、各类 period/date 的数字与格式、template、theme、settings、sections 的 key 与 hidden；
...
type Project = {name:string;role?:string;period?:string;tech?:string[];highlights?:string[];link?:string};
type ResumeData = { label:string; target?:string; template?:'classic'|'sidebar'|'compact'; theme?:'blue'|'emerald'|'violet'|'rose'|'slate'|'amber'|'cyan'|'fuchsia'; sections?:{key:'summary'|'education'|'work'|'internship'|'projects'|'skills'|'awards'|'certificates'|'languages'|'activities'|'interests'|'custom';title?:string;hidden?:boolean;customId?:string}[]; custom?:{id:string;title:string;content?:string}[]; basics:{name:string;title?:string;email?:string;phone?:string;wechat?:string;location?:string;birth?:string;political?:string;website?:string;github?:string;summary?:string}; education?:{school:string;college?:string;degree?:string;major?:string;period?:string;gpa?:string;courses?:string;detail?:string}[]; work?:{company:string;position?:string;period?:string;location?:string;highlights?:string[];projects?:Project[]}[]; internship?:{company:string;position?:string;period?:string;location?:string;highlights?:string[];projects?:Project[]}[]; projects?:Project[]; skills?:string; awards?:{title:string;issuer?:string;date?:string}[]; certificates?:{name:string;issuer?:string;date?:string}[]; languages?:{name:string;level?:string}[]; activities?:{name:string;role?:string;period?:string;highlights?:string[]}[]; interests?:string[] };
规则：
