...
- **在线简历编辑器** —— 超级简历式双栏编辑（分区表单 + 实时预览），富文本工具栏（加粗/斜体/下划线/删除线/代码、序列号/箭头列表/引用/链接、字号/颜色/对齐）、多模板（经典/紧凑/卡片/双栏侧边）、多配色、文本格式（条目标题单行/双行，字段分隔符 `·` `/` `|` 或分散对齐）、智能一页（自动压缩排版塞进一页）、真·多页 A4 预览、浏览器打印导出 PDF、导出 Word、导入简历、翻译成英文。改动实时存本地浏览器（localStorage 草稿）。
