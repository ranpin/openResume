...
- **在线简历编辑器** —— 超级简历式双栏编辑（分区表单 + 实时预览）。全局设置集中在顶部工具栏：「模板」「配色」「排版」（字号/行距/间距/页边距/字体/标题行数/字段样式）「模块」（模块管理：拖拽调序、改名、显隐）下拉面板、「智能一页」（自动压缩排版塞进一页）、页数显示、撤销/重做、预览模式（隐藏表单、预览占满）、保存、统一「导出」（PDF 打印 / Word / YAML 数据）、发布到线上。富文本工具栏（加粗/斜体/下划线/删除线/代码、序列号/箭头列表/引用/链接、字号/颜色/对齐）、多模板（经典/紧凑/卡片/双栏侧边）、多配色、文本格式（条目标题单行/双行，字段分隔符 `·` `/` `|` 或分散对齐）、真·多页 A4 预览（随面板宽度自适应缩放，窄屏不溢出）、导入简历、翻译成英文。改动实时存本地浏览器（localStorage 草稿）。
