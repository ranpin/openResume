...
>
> **全局排版 / 文本格式 / 证件照 / 模块管理**：网页编辑器顶部工具栏的「排版」下拉面板提供全局排版滑块（字号/行距/间距/页边距）与正文字体、标题行数、字段样式；「模板」「配色」也在同一栏切换；文本格式（条目标题单行/双行；字段用 `·` `/` `|` 分隔或「分散对齐」——首字段贴左、时间贴右、中间字段在一行内均分，靠 flex `justify-between` 实现两端对齐）、**智能一页**（按当前页数自动逐级压缩字号/行距/间距/页边距，直到塞进一页或到达下限）、「基本信息」里的证件照上传（自动压缩内嵌）、以及「模块管理」（拖拽调序、改模块名、显隐）照旧。这些都会写进本 YAML 的 `settings` / `basics.photo` / `sections` 字段。所有新字段均可选，旧简历不填也照常渲染。
